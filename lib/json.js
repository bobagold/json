...
function main(argv) {
    ...
    function parseChunk(chunk, obj, filename, inPlace, headers) {
        ...
        if (!exe) {
            ...
            for (i = 0; i < data.length; i++) {
                ...
                for (j = 0; j < exeScripts.length; j++) {
                    exeScripts[j].runInNewContext(datum, {timeout: 3000});
                }
            }
            if (arrayified) {
                data = data[0];
            ...
            for (j = 0; j < exeScripts.length; j++) {
                exeScripts[j].runInNewContext(data, {timeout: 3000});
            }
        }
        ...
        if (!cond) {
            ...
            for (i = 0; i < data.length; i++) {
                ...
                if (keep) {
                    for (j = 0; j < condScripts.length; j++) {
                        if (!condScripts[j].runInNewContext(datumCopy, {timeout: 3000})) {
                            keep = false;
                            break;
                        }
                    }
            ...
            if (keep) {
                for (j = 0; j < condScripts.length; j++) {
                    if (!condScripts[j].runInNewContext(dataCopy, {timeout: 3000})) {
                        keep = false;
                        break;
                    }
                }
